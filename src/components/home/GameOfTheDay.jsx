...
function WordleGame({ onComplete, wordList }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
          ...
          return (
            <div key={row} className={`flex gap-1.5 ${isActive && shake ? "animate-bounce" : ""}`}>
              {Array.from({ length: WORD_LENGTH }).map((_, col) => {
                ...
                return (
                  <div key={col} className={`w-11 h-11 border-2 rounded-lg flex items-center justify-center text-base font-bold uppercase transition-all ${colorClass}`}>
                    {letter}
                  </div>
                );
              })}
      ...
      {/* Status */}
      {status === "won" && (
        <div className="text-center mb-4 p-3 bg-emerald-50 rounded-xl">
          <p className="text-emerald-700 font-bold text-sm">🎉 You got it in {guesses.length} {guesses.length === 1 ? "try" : "tries"}!</p>
        </div>
      )}
      {status === "lost" && (
        <div className="text-center mb-4 p-3 bg-red-50 rounded-xl">
          <p className="text-red-700 font-bold text-sm">The word was <span className="uppercase">{TARGET}</span>. Try again tomorrow!</p>
        </div>
      )}
            ...
            {row.map(key => {
              ...
              return (
                <button
                  ...
                  className={`${isSpecial ? "px-3 text-xs h-10" : "w-8 h-10"} rounded-lg font-bold text-xs uppercase transition-colors ${isSpecial ? specialClass : state ? keyColors[state] : "bg-gray-200 text-gray-800 hover:bg-gray-300"}`}
                >
                  {key}
                </button>
              );
