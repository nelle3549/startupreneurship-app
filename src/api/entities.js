 ...
 *
 * Base44 API:
 *   entities.User.list(orderBy)     → SELECT * FROM users ORDER BY ...
 *   entities.User.filter({email})   → SELECT * FROM users WHERE email = ...
 *   entities.User.create({...})     → INSERT INTO users ...
 *   entities.User.update(id, {...}) → UPDATE users SET ... WHERE id = ...
 *   entities.User.delete(id)        → DELETE FROM users WHERE id = ...
 */
...
/**
 * Entity proxy object — access entities like entities.User.filter(...)
